...
import 'admin_get_all_orders.dart';
...
class _AdminUploadItemsScreenState extends State<AdminUploadItemsScreen> {
  ...
  captureImageWithPhoneCamera()async{
   //when user click on this button from the dialog box captured with phone camera
  ...
  pickeImageFromPhoneGallery()async{
  ...
  //defaultScreen methods ends her
  ...
  uploadItemImage()async{
    //using http sending tha request to the imgur api
    var requestImgurApi= http.MultipartRequest(
      "POST",
      Uri.parse("https://api.imgur.com/3/image")
    );

    //unique name for imagename
    String imageName=DateTime.now().microsecondsSinceEpoch.toString();

    requestImgurApi.fields['title']=imageName;
    requestImgurApi.headers['Authorization']="Client-ID " + "cc2d27cbb3c247e";

    //image path and the file
    ...
    //upload this file so we can request
    //we are sending a reques to the imgur api
    ...
    //this responseDataFromImgurApi in bytes form so we should convert it in readable form
    var resultFromImgurApi=  String.fromCharCodes(responseDataFromImgurApi);
    print("Result::");
    print(resultFromImgurApi);

    //decode tha data response from api and get image link from json response data
    Map<String,dynamic> jsonRes= json.decode(resultFromImgurApi);
    //this is in key value pare -> key is always in string format
    //and dynamic means that is the value can be in any format
   ...
   String deleteHash=(jsonRes["data"]["deletehash"]).toString();
    ...
    //upload and save new items to mysqldb
  ...
  saveItemInfoToDatabase()async{
    ...
    List<String> colorsList=colorsController.text.split(','); //red,white,black
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (resBodyOfUploadItem['success'] == true) {
          //it comes with a value true,it means the new item upload successfully
